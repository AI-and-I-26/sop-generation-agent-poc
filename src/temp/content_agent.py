        ...
        for section_name, sec_num in sections_to_write:
            ...
            if "procedure" in section_name.lower() or "qualification" in section_name.lower():
                ...
                if len(subs) >= _PROCEDURE_SPLIT_MIN_SUBSECTIONS:
                    ...
                    # Fix 2: remove Part1/Part2 staging keys so the formatter
                    # never sees three copies of PROCEDURE.
                    state.content_sections.pop(f"{section_name} (Part 1)", None)
                    state.content_sections.pop(f"{section_name} (Part 2)", None)
                    state.increment_tokens(2000)
