...
CRITICAL — KB SECTION TITLE CONSTRAINT:
When kb_format_context.section_titles is provided, use those exact top-level
titles. Do NOT substitute topic-specific paraphrases for structural names.

  WRONG: "6.0 IT Infrastructure Qualification Steps"
  RIGHT: "6.0 PROCEDURE"

  WRONG: "3.0 Team Roles and Accountabilities"
  RIGHT: "3.0 RESPONSIBILITIES"

Only Procedure subsections (6.x) may carry topic-appropriate titles.
Every top-level title must match the KB exactly.
...
10. RESPONSIBILITIES TABLE — each role must appear EXACTLY ONCE.
    Merge multiple facts about the same role into one row.

    WRONG: {"role": "IT Manager", "responsibility": "Oversees qualification"}
           {"role": "IT Manager", "responsibility": "Signs test evidence"}

    RIGHT: {"role": "IT Manager", "responsibility": "Oversees qualification and signs test evidence"}

11. SCOPE vs PROCEDURE — strictly separate:
    SCOPE: states WHAT is covered and excluded. No procedural steps, no tool
    names, no DocuSign/ServiceNow actions, no approval routing.
    PROCEDURE: HOW each step is performed. Move any action sentence here.

    WRONG in Scope: "DocuSign routes the qualification record to the approver."
    RIGHT in Scope: "Electronic approval workflows are within scope."
    RIGHT in Procedure: "Route the qualification record via DocuSign to the
      designated approver for e-signature per 21 CFR Part 11."
...
2. Tables: Markdown pipe syntax | col | col |
   BANNED: ** bold markers inside any table cell — header or data.

   WRONG: | **Role** | **Responsibility** |
          | **IT Manager** | Manages **all** activities |

   RIGHT: | Role | Responsibility |
          | IT Manager | Manages all activities |

   Strip ALL ** markers from table cell content before outputting.
