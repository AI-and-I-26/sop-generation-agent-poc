...
def create_sop_workflow():
    ...
    """
    # max_iterations=15: 5 nodes x 3 passes = hard ceiling.
    # Eliminates "Graph without execution limits" WARNING.
    # The should_revise() predicate (retry_count < 2) is the semantic
    # guard; max_iterations is the structural safety net behind it.
    gb = GraphBuilder(max_iterations=15)
...
# ---------------------------------------------------------------------------
# TEMPLATE VARIABLE SCRUBBER (Fix 1)
# Replaces any {{placeholder}} tokens the formatter left unreplaced
# (status, classification, next_review, etc.) with safe readable defaults
...
def _scrub_template_vars(text: str) -> str:
    """Replace remaining {{key}} tokens with defaults."""
    def _replace(m: re.Match) -> str:
        key = m.group(1).strip().lower()
        return _TEMPLATE_DEFAULTS.get(key, key.upper())
...
async def generate_sop(
    ...
    try:
        ...
        # Fix 1: scrub unreplaced {{placeholder}} tokens before QA sees them
        for _attr in ("formatted_markdown", "formatted_document"):
            _val = getattr(final_state, _attr, None)
            if _val:
                setattr(final_state, _attr, _scrub_template_vars(_val))
        STATE_STORE[workflow_id] = final_state
