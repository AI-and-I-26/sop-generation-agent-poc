...
def _bedrock_model(env_var: str, max_tokens: int = 2048) -> BedrockModel:
    return BedrockModel(model_id=os.getenv(env_var, _DEFAULT_ARN), max_tokens=max_tokens)
...
def _make_llm_agent() -> Agent:
    return Agent(
        name="QALLM",
        model=_bedrock_model("MODEL_QA", max_tokens=2048),
